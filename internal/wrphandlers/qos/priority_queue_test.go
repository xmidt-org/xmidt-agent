...
func testEnqueueDequeueAgePriority(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			...
			var err error
			pq.tieBreaker, pq.trimQueue.tieBreaker, err = priority(tc.priority)
			require.NoError(err)
			...
			assert.Equal(pq.sizeBytes, pq.trimQueue.sizeBytes)
		})
	}
}
...
func testEnqueueDequeue(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			...
			var err error
			pq.tieBreaker, pq.trimQueue.tieBreaker, err = priority(NewestType)
			require.NoError(err)
...
func testSize(t *testing.T) {
	...
	assert.Equal(int64(0), pq.sizeBytes)
	itm := item{msg: msg, timestamp: time.Now()}
	pq.Push(&itm)
	pq.Push(&itm)
...
func testPush(t *testing.T) {
	...
	for _, msg := range messages {
		pq.Push(&item{msg: msg, timestamp: time.Now()})
